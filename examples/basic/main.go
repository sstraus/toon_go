...
func main() {
	...
	encoded, err := toon.Marshal(obj, nil)
	if err != nil {
		log.Fatal(err)
	}
	...
	encoded, _ = toon.Marshal(nested, nil)
	fmt.Println(string(encoded))
	...
	encoded, _ = toon.Marshal(inlineArr, nil)
	fmt.Println(string(encoded))
	...
	encoded, _ = toon.Marshal(tabular, nil)
	fmt.Println(string(encoded))
	...
	encoded, _ = toon.Marshal(listArr, nil)
	fmt.Println(string(encoded))
	...
	encoded, _ = toon.Marshal(withOpts, opts)
	fmt.Println(string(encoded))
	...
	encoded, _ = toon.Marshal(complex, nil)
	fmt.Println(string(encoded))
	...
	toonEncoded, _ := toon.Marshal(data, nil)
	fmt.Printf("TOON (%d chars):\n%s\n", len(toonEncoded), string(toonEncoded))
